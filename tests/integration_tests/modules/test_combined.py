    ...
    def test_rsyslog(self, class_client: IntegrationInstance):
        ...
        if class_client.execute("command -v rsyslogd").failed:
            ...
            verify_clean_boot(
                ...
                ignore_warnings=True,  # Ignore other expected warnings
                ignore_deprecations=True,  # Ignore exepected deprecations
            )
            class_client.execute("cloud-init status --format=yaml")
        else:
